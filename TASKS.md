...
- [x] **V18.2: Chromecast HLS Hardening** (2026-04-20):
  ...
  - **Local Transition Prebuffer Foundation**: `PlaybackManager` now prepares the next local HLS track with a secondary `HTMLAudioElement` + hls.js instance, promotes that prepared element when the next track is requested, and logs visible preload/readiness/promotion/track-end-to-audible-start transition telemetry to the browser console. Transient Cast connection state is no longer persisted so stale session flags cannot suppress local prebuffering after reloads.
