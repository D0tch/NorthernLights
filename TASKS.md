...
- [x] **V18.2: Chromecast HLS Hardening** (2026-04-20):
  ...
  - **Receiver UI Performance Guardrails**: Optimized the custom receiver UI render loop so progress updates are throttled, DOM writes are diff-based, artwork/background updates only happen on image changes, queue rows are rebuilt only when the queue key changes, and hidden OK-level debug logs no longer append DOM nodes. This addresses Cast debugger `timeupdate`/`setTimeout` long-task stalls while audio continues playing.
