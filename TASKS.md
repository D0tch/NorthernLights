...
- [x] **V18.2: Chromecast HLS Hardening** (2026-04-20):
  ...
  - **Local Transition Prebuffer Foundation**: `PlaybackManager` now prepares the next local HLS track with a secondary `HTMLAudioElement` + hls.js instance, promotes that prepared element when the next track is requested, and logs track-end-to-audible-start transition latency to the browser console. This is the foundation for later gapless/crossfade work.
